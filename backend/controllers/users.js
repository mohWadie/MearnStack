...
router.post('/register', async (req, res) => {
    try{
        ...
        let dob2 = dob.split('/');
        req.body.DOB = new Date(Number(dob2[2]), Number(dob2[1]) - 1, Number(dob2[0]) + 1);
        ...
        if(userExists || emailExists) return res.status(400).send("username or email already exists");
